...
def convert_prob_to_label(prob_folder, ensemble_label_path, input_folder_path):
    """
    Converts probabilistic segmentation maps to discrete label maps.
    #IN:
        prob_folder: folder containing .npz files with softmax probabilities
        ensemble_label_path: folder to save the final .nii.gz segmentations
        input_folder_path: folder with original NIfTI files for header metadata
    #OUT:
        Saves .nii.gz files with label maps in `ensemble_label_path`
    """
    prob_files_list = listdir(prob_folder)
    print(f"Found {len(prob_files_list)} probability files to convert.")

    for prob_file in prob_files_list:
        if not prob_file.endswith(".npz"):
            continue  # skip non-npz files

        prob_file_path = join(prob_folder, prob_file)
        case_id = prob_file.replace(".npz", "")
        print(f"Processing: {case_id}")

        # Load probability map
        predicted_probabilities = np.load(prob_file_path)['probabilities']  # shape: (3, H, W, D)

        # Convert to segmentation: 0: background, 1: NETC, 2: SNFH, 3: ET
        segmentation = np.zeros(predicted_probabilities.shape[1:], dtype=np.uint16)
        for i, label in enumerate([0, 1, 2, 3]):  # channel index to label mapping
            segmentation[predicted_probabilities[i] > 0.5] = label

        # Reorder to match original orientation (transpose ZYX to XYZ)
        segmentation = segmentation.transpose(2, 1, 0)

        # Load corresponding header metadata from one of the input modalities
        input_nii_path = join(input_folder_path, f"{case_id}_0000.nii.gz")
        pred_nii = nib.load(input_nii_path)

        # Save output with original header info
        output_nii = nib.Nifti1Image(segmentation, affine=pred_nii.affine, header=pred_nii.header)
        output_path = join(ensemble_label_path, f"{case_id}.nii.gz")
...
def perform_inference_step(inference_folder, input_folder_nnunet, ensemble_code):
    ...
    if ensemble_code not in model_map:
        raise ValueError(f"Unknown ensemble_code: {ensemble_code}")

    trainer_folder = model_map[ensemble_code]
    results_path = join(os.environ['nnUNet_results'], "Dataset601_BraTS", trainer_folder)  # <- use your mount path structure
    ...
    output_folder = join(inference_folder, "ensemble", ensemble_code, "tmp")
    maybe_mkdir_p(output_folder)

    trainer = trainer_folder.split('__')[0]
    config = trainer_folder.split('__')[-1]

    command = f"nnUNetv2_predict -d {dataset_id} -i {input_folder_nnunet} -o {output_folder} -f all -tr {trainer} -c {config} -p nnUNetResEncUNetMPlans -device cuda --save_probabilities --disable_tta"
    print("Running:")
    print(command)
    ...
    proc = subprocess.Popen(
        ...
        stderr=subprocess.STDOUT,  # Redirect stderr to stdout so you can see all errors
        text=True,
    ...
    for line in proc.stdout:
        ...
        stdout_log.append(line)     # Save for reference/logging

    proc.wait()  # Wait for process to complete

    # Mimic subprocess.run-style result
    class Result:
        def __init__(self, returncode, stdout):
            self.returncode = returncode
            self.stdout = stdout
            self.stderr = None  # because we redirected it
    ...
    # Ensemble step (with just 1 model, acts as passthrough)
    ensemble_prob_path = join(inference_folder, 'ensemble', ensemble_code, 'prob')
    maybe_mkdir_p(ensemble_prob_path)
